--[[
    GamesenseUI - UI library for Roblox LocalScripts
    Style: gamesense-inspired flat dark UI
    Features: draggable window, horizontal/vertical tabs, open/close toggle with animation,
              checkboxes, sliders, dropdowns, buttons, custom notifications, mobile support.

    This module contains ONLY UI construction code. No gameplay logic.
--]]
...
local Theme = {
    ...
    TopBarGradient  = {Color3.fromRGB(0, 200, 220), Color3.fromRGB(255, 0, 200), Color3.fromRGB(255, 230, 0)},
    ...
    SliderFill      = Color3.fromRGB(0, 190, 255),
...
function Library.new(title)
    ...
    -- Body container (holds tab-bar + pages), layout depends on orientation
    ...
    -- Keybind to toggle (Insert key on PC)
...
function Library:SetOrientation(orientation)
    ...
    if orientation == "Horizontal" then
        self.TabBar.Size = UDim2.new(1, 0, 0, 34)
        self.TabBar.Position = UDim2.new(0, 0, 0, 0)
        self.TabBarLayout.FillDirection = Enum.FillDirection.Horizontal

        self.PageContainer.Size = UDim2.new(1, 0, 1, -34)
        self.PageContainer.Position = UDim2.new(0, 0, 0, 34)
    else
        self.TabBar.Size = UDim2.new(0, 130, 1, 0)
        self.TabBar.Position = UDim2.new(0, 0, 0, 0)
    ...
    for _, tabData in ipairs(self.Tabs) do
        if orientation == "Horizontal" then
            tabData.Button.Size = UDim2.new(0, 90, 1, 0)
        else
            tabData.Button.Size = UDim2.new(1, 0, 0, 34)
        end
    end
end
...
function Library:Toggle()
    ...
    if self.Open then
        ...
        local targetSize = IsMobile and UDim2.fromOffset(380, 300) or UDim2.fromOffset(640, 400)
...
function Library:CreateTab(name, icon)
    ...
    local button = create("TextButton", {
        Name = name .. "Button",
        Size = self.Orientation == "Horizontal" and UDim2.new(0, 90, 1, 0) or UDim2.new(1, 0, 0, 34),
    ...
    -- Two-column layout using two frames side by side
...
function TabAPI:AddSection(text, side)
    ...
    local holder = create("Frame", {
        ...
        BackgroundColor3 = Theme.Panel,
        BorderSizePixel = 0,
        LayoutOrder = column:GetChildren() and #column:GetChildren() or 0,
        Parent = column,
    })
    corner(4).Parent = holder
...
--=============================
-- SECTION API (elements within a section)
--=============================
SectionAPI = {}
...
function SectionAPI:AddCheckbox(text, default, callback)
    ...
    clickArea.MouseButton1Click:Connect(function()
        state = not state
        tween(box, TweenInfo.new(0.12), {BackgroundColor3 = state and Theme.ToggleOn or Theme.ToggleOff})
        callback(state)
    end)
...
function SectionAPI:AddButton(text, callback)
    ...
    btn.MouseEnter:Connect(function()
        tween(btn, TweenInfo.new(0.12), {BackgroundColor3 = Theme.TabActive})
    end)
...
function Library:Notify(title, text, duration, notifType)
    ...
    tween(notif, TweenInfo.new(0.28, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
        Position = UDim2.new(0, 0, 0, 0),
    })

    task.delay(duration, function()
        local ok = pcall(function()
            local fadeTween = tween(notif, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
